...
import com.tomsyb.lib_base.ext.singleClick
...
class StateView @JvmOverloads constructor(
    ...
    var animatorProvider: AnimatorProvider? = FadeAlphaAnimatorProvider()
        set(value) {
            field = value
    ...
    init {
        ...
        if (emptyResource == 0) {
            emptyResource = R.layout.include_empty_view
        }
        if (retryResource == 0) {
            retryResource = R.layout.include_empty_view
        }
        if (loadingResource == 0) {
            loadingResource = R.layout.include_progress_view
        }
    ...
    fun isLoading():Boolean{
        val view = views[loadingResource]
    ...
    companion object {
        ...
        fun inject(activity: Activity): StateView {
            return inject(activity.window.decorView.findViewById<View>(R.id.content))
        }
